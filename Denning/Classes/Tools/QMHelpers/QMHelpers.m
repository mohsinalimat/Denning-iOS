...
NSString* getTimeLog(NSString* date) {
    ...
    if (conversionInfo.year > 0) {
        timeLog = [NSString stringWithFormat:@"%ld year(s) ago", (long)conversionInfo.year];
    } else if (conversionInfo.month > 0) {
        timeLog = [NSString stringWithFormat:@"%ld month ago", (long)conversionInfo.month];
    } else if (conversionInfo.day > 0) {
        timeLog = [NSString stringWithFormat:@"%ld day(s) ago", (long)conversionInfo.day];
    } else if (conversionInfo.hour > 0) {
        timeLog = [NSString stringWithFormat:@"%ld hours ago", (long)conversionInfo.hour];
    } else if (conversionInfo.minute ) {
        timeLog = [NSString stringWithFormat:@"%ld minutes ago", (long)conversionInfo.minute ];
    } else  if (conversionInfo.second ){
        timeLog = [NSString stringWithFormat:@"%ld seconds ago", (long)conversionInfo.second ];
    } else {
        timeLog = @"Now";
    }
    return timeLog;
