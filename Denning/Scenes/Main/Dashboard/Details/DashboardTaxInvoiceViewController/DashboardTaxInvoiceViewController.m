//
//  DashboardTaxInvoiceViewController.m
//  Denning
//
...
#import "DashboardTaxInvoiceCell.h"
#import "ThreeColumnSecondCell.h"

@interface DashboardTaxInvoiceViewController ()
...
@implementation DashboardTaxInvoiceViewController
...
- (void)registerNibs {
    [DashboardTaxInvoiceCell registerForReuseInTableView:self.tableView];
...
- (void) getList {
    ...
    [[QMNetworkManager sharedManager] getDashboardTaxInvoiceInURL:self.url withPage:self.page withFilter:self.filter withCompletion:^(NSArray * _Nonnull result, NSError * _Nonnull error) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    TaxInvoceModel *model = self.listOfProperties[indexPath.row];
    if (indexPath.row == 0) {
        DashboardTaxInvoiceCell *cell = [tableView dequeueReusableCellWithIdentifier:[DashboardTaxInvoiceCell cellIdentifier] forIndexPath:indexPath];
        
        cell.fileNo.text = model.fileNo;
        cell.fileName.text = model.fileName;
        cell.taxInvoiceNo.text = model.invoiceNo;
        cell.openDate.text = model.amount;
        return cell;
    }
    ...
    cell.fileNo.text = model.fileNo;
    cell.fileName.text = model.fileName;
    cell.invoiceNo.text = model.invoiceNo;
