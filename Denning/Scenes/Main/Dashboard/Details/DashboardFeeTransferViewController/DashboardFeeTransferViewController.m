//
//  DashboardFeeTransferViewController.m
//  Denning
//
...
#import "FeeTransferCell.h"
#import "BankCashSecondCell.h"

@interface DashboardFeeTransferViewController ()
...
@implementation DashboardFeeTransferViewController
...
- (void)registerNibs {
    [FeeTransferCell registerForReuseInTableView:self.tableView];
...
- (void) getList {
    ...
    [[QMNetworkManager sharedManager] getDashboardFeeTransferInURL:self.url withPage:self.page withFilter:self.filter withCompletion:^(NSArray * _Nonnull result, NSError * _Nonnull error) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    FeeTranserModel *model = self.listOfProperties[indexPath.row];
    if (indexPath.row == 0) {
        FeeTransferCell *cell = [tableView dequeueReusableCellWithIdentifier:[FeeTransferCell cellIdentifier] forIndexPath:indexPath];
        
        cell.dateLabel.text = model.batchDate;
        cell.batchLabel.text = model.batchNo;
        cell.amountLabel.text = model.totalAmount;
        return cell;
    }
    ...
    cell.firstLabel.text = model.batchDate;
    cell.middleLabel.text = model.batchNo;
    cell.rightLabel.text = model.totalAmount;
